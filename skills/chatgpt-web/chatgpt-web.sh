...
check_chatgpt_window() {
    local WINDOWS=$(cicy-rpc get_windows 2>/dev/null)
    if echo "$WINDOWS" | grep -q "chatgpt.com"; then
        echo "ChatGPT 窗口已存在"
        return 0
    else
        echo "未找到 ChatGPT 窗口，正在打开..."
        cicy-rpc open_window url="https://chatgpt.com"
        sleep 5
        return 1
    fi
}
...
check_login() {
    local STATUS=$(cicy-rpc chatgpt_web_status 2>/dev/null)
...
case "$COMMAND" in
    ...
    conversations)
        ...
        LIMIT=${1:-10}
        STRIP=${2:-0}
        cicy-rpc get_chatgpt_web_conversations limit="$LIMIT" strip="$STRIP"
        ;;
    messages)
        check_chatgpt_window
        wait_dom_ready
    ...
    messages-by-id)
        ...
        wait_dom_ready
        CONV_ID=$1
        cicy-rpc get_chatgpt_web_messages_by_id conversation_id="$CONV_ID"
        ;;
    ask)
        check_chatgpt_window
        wait_dom_ready
        check_login || exit 1
        TEXT="$*"
        RESULT=$(cicy-rpc chatgpt_web_ask text="$TEXT" 2>/dev/null)
    ...
    set-prompt)
        ...
        wait_dom_ready
        TEXT="$*"
        cicy-rpc chatgpt_web_set_prompt text="$TEXT"
        ;;
    send)
        check_chatgpt_window
        wait_dom_ready
        cicy-rpc chatgpt_web_click_send
        ;;
    clear)
        check_chatgpt_window
        wait_dom_ready
        cicy-rpc chatgpt_web_clear_prompt
        ;;
    open)
        CONV_ID=$1
        if [ -z "$CONV_ID" ]; then
        ...
        cicy-rpc open_chatgpt_web_chat_by_id conversation_id="$CONV_ID"
        ;;
    is-logged)
        check_chatgpt_window
        wait_dom_ready
        cicy-rpc is_chatgpt_logged
        ;;
    help|--help|-h)
